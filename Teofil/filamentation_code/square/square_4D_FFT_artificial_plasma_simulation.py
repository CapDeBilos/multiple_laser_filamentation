...
Run example:
    python square_4D_FFT_artificial_plasma_simulation.py 1.1 2.0
"""
...
def run_one_pin(pin_factor, outdir):
    ...
    # Square transverse beam. Start from zeros, then fill the central square.
    # The amplitude is normalized so that sum(|E|^2) * DeltaX * DeltaY ~= Pin.
    E = cp.zeros((Nx, Ny), dtype=cp.complex128)
    number_1 = int(0.45 * Nx)
    number_2 = int(0.55 * Nx)
    number_3 = int(0.45 * Ny)
    number_4 = int(0.55 * Ny)
    square_area = (number_2 - number_1) * DeltaX * (number_4 - number_3) * DeltaY
    amp = cp.sqrt(Pin / square_area)
    E[number_1:number_2, number_3:number_4] = amp
    ...
    npz_path = os.path.join(outdir, f"Pin_{tag}_Pcr_square_4D_FFT_diagnostics.npz")
    png_path = os.path.join(outdir, f"Pin_{tag}_Pcr_square_4D_FFT_diagnostics.png")
    ...
    np.savez(
        ...
        I_snapshot_z=np.array(I_snapshot_z),
        I_snapshot_steps=np.array(I_snapshot_steps),
        I_snapshot_Nx=np.array(I_snapshot_Nx),
        I_snapshot_Ny=np.array(I_snapshot_Ny),
        I_snapshot_x=as_object_array(I_snapshot_x),
        I_snapshot_y=as_object_array(I_snapshot_y),
        I_snapshots=as_object_array(I_snapshots),
        full_I_save_every=full_I_save_every,
        x=x_cpu,
        y=y_cpu,
        I_final=I_final,
        rho_final=rho_final,
    ...
    plt.grid(True)
    plt.legend()
    plt.title(f"Square beam, Pin/Pcr = {pin_factor:.1f}")
    plt.tight_layout()
