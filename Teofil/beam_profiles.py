...
def supergaussian_intensity(r, Pin=1, width=0.5, p=2):
    '''
    Pin - input power
    p - the order of the supergaussian, use p=2 and you get normal gaussian
    '''
    I0 = p / (2 * np.pi * width ** 2) * Pin * 1 / gamma(2 / p)
    return I0 * np.exp(- (np.abs(r) / width) ** p)

def gaussian_intensity(r, Pin=1, width=0.5):
    return supergaussian_intensity(r, Pin, width, p=2)
...
def supergaussian(r, Pin=1, width=0.5, p=2):
    ...
    p - the order of the supergaussian, use p=2 and you get normal gaussian
    '''
    E = np.sqrt(Pin * p * 2 ** (2/p) / (np.pi * lightspeed * eps0 * width**2 * gamma(2/p)))
