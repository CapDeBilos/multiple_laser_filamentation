...
coordinates (x, y), with propagation along z and no explicit time coordinate.
...
Run example:
    python THOMAS_PCR_adaptive_FFT_sweep_xy_no_t_memory_optimized_noise.py 1.1 2.0
"""
...
# Initial noise controls
# Noise is applied once at z = 0 to seed transverse imperfections.
# Both noises are band-limited in Fourier space to avoid artificial high-frequency growth.
NOISE_ON = True
NOISE_SEED = 12345

# Multiplicative amplitude noise: E -> E * (1 + a_noise)
MULTIPLICATIVE_NOISE_ON = True
amp_noise_rms = 1.0e-3

# Phase-only noise: E -> E * exp(i * phi_noise)
PHASE_NOISE_ON = True
phase_noise_rms = 5.0e-2  # radians

# Correlation lengths for the noise, expressed as fractions of the beam waist.
# Values around 0.1 to 0.2 are a reasonable starting point.
noise_corr_x_factor = 0.10
noise_corr_y_factor = 0.10

# Optional safety clip for multiplicative noise. Prevents rare Gaussian outliers
# from creating local unphysical hot spots. Set to None to disable.
amp_noise_clip_sigma = 4.0
...
def _rng_standard_normal(rng, shape, dtype=cp.float64):
    """CuPy-version-tolerant standard normal generator."""
    try:
...
def _make_cupy_rng(seed):
    """Use default_rng when available, otherwise fall back to RandomState."""
    if hasattr(cp.random, "default_rng"):
        return cp.random.default_rng(seed)
    return cp.random.RandomState(seed)


def _normalized_band_limited_noise(shape, kx_vector, ky_vector, corr_x, corr_y, rng, dtype=cp.float64):
    """Return real, zero-mean, unit-RMS, band-limited 2D noise on the GPU.

    White noise contains grid-scale components that can be strongly amplified by
    nonlinear propagation or remeshing. This function low-pass filters the noise
    in k-space with a Gaussian envelope before normalization.
    """
    raw = _rng_standard_normal(rng, shape, dtype=dtype)

    # Gaussian low-pass. The exponent is dimensionless: k [1/m] times corr [m].
    # Large corr -> stronger suppression of high spatial frequencies.
    filt = cp.exp(-0.5 * ((kx_vector[:, None] * corr_x) ** 2 + (ky_vector[None, :] * corr_y) ** 2))
    smooth = cp.fft.ifft2(cp.fft.fft2(raw) * filt).real

    smooth -= cp.mean(smooth)
    rms = cp.sqrt(cp.mean(smooth * smooth))
    if float(rms.get()) > 0.0:
        smooth /= rms
    return cp.ascontiguousarray(smooth)


def apply_initial_noise(E, kx_vector, ky_vector, pin_factor):
    """Apply band-limited multiplicative amplitude noise and phase noise once at z=0."""
    if not NOISE_ON:
        return E

    # Make each Pin/Pcr reproducible but different.
    seed = int(NOISE_SEED + round(1000.0 * pin_factor))
    rng = _make_cupy_rng(seed)

    corr_x = noise_corr_x_factor * waist
    corr_y = noise_corr_y_factor * waist

    if MULTIPLICATIVE_NOISE_ON and amp_noise_rms > 0.0:
        amp_noise = _normalized_band_limited_noise(E.shape, kx_vector, ky_vector, corr_x, corr_y, rng)
        if amp_noise_clip_sigma is not None:
            amp_noise = cp.clip(amp_noise, -float(amp_noise_clip_sigma), float(amp_noise_clip_sigma))
        E = E * (1.0 + amp_noise_rms * amp_noise)

    if PHASE_NOISE_ON and phase_noise_rms > 0.0:
        phase_noise = _normalized_band_limited_noise(E.shape, kx_vector, ky_vector, corr_x, corr_y, rng)
        E = E * cp.exp(1j * phase_noise_rms * phase_noise)

    return cp.ascontiguousarray(E)
...
def run_one_pin(pin_factor, outdir):
    ...
    # 2D Gaussian transverse beam. With this convention, integral |E|^2 dx dy ~= Pin.
    amp = cp.sqrt(2.0 * Pin / (cp.pi * waist**2))
    E = amp * cp.exp(-(x_vector[:, None] ** 2 + y_vector[None, :] ** 2) / waist**2)
    E = cp.ascontiguousarray(E.astype(cp.complex128))

    # Seed realistic transverse imperfections without injecting grid-scale noise.
    E = apply_initial_noise(E, kx_vector, ky_vector, pin_factor)
    ...
    I_center = [float(I[Nx // 2, Ny // 2].get())]
    ...
    while z < zmax - 1e-15:
        ...
        if step % number_diag == 0 or z >= zmax - 1e-15:
            ...
            I_center.append(float(I[Nx // 2, Ny // 2].get()))
            ...
            print(
                ...
                f"I_center/I0={I_center[-1]/I_center[0]:.3e}, "
    ...
    npz_path = os.path.join(outdir, f"Pin_{tag}_Pcr_xy_no_t_memory_optimized_noise.npz")
    png_path = os.path.join(outdir, f"Pin_{tag}_Pcr_xy_no_t_memory_optimized_noise.png")
    ...
    plt.plot(np.array(z_diag), np.array(I_center) / I_center[0], "o-", label="I(x=0,y=0) / I0")
    plt.xlabel("z (m)")
...
def main():
    ...
    outdir = f"pin_sweep_xy_no_t_memory_optimized_noise_{sweep}"
